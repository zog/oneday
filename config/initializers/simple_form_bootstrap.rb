  ...
  module Wrappers
    class Root < Many
      def html_classes(input, options)
        ...
        css << (options[:wrapper_success_class] || @defaults[:success_class]) if input.object.present? && input.object.was_validated? && !input.has_errors? && input.send(:required_by_validators?)
